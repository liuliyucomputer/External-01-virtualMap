...
class Student:
    ...
    def update_position(self, current_time: float):
        """更新学生位置"""
        if not self.current_path or self.move_start_time is None:
            return
        ...
        if progress >= 1.0:
            ...
            if self.current_path:
                # 检查新路径起始点和刚走完路径的结束点坐标是否一致
                next_path = self.current_path[0]
                if self.position!= next_path.start:
                    print(f"路径切换错误：学生 {self.name} 坐标不匹配，当前位置 {self.position} 与新路径起始点 {next_path.start} 不一致")
                    return
                self.move_start_time = current_time  # 重置开始时间
            else:
                self.destination = None  # 到达最终目的地
...
class Game:
    ...
    def find_shortest_path(self, start: Point, end: Point) -> List[Path]:
        ...
        for path in self.paths:
            ...
            # 只有正向路径不存在时才添加反向路径（可根据实际情况调整添加反向路径的条件）
