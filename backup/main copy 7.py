...
class Game:
    ...
    def find_shortest_path(self, start: Point, end: Point) -> List[Path]:
        ...
        for path in self.paths:
            ...
            graph[path.end].append((path.start, path))  # 假设双向路径
        ...
        while queue:
            ...
            for neighbor, edge in graph[node]:
                if neighbor not in visited:
                    heapq.heappush(queue, (cost + edge.length, neighbor, path + [edge]))
    ...
    def update_student_activity(self, student):
        ...
        if not student.schedule:
            return
        current_activity = student.schedule[0]
        if student.position == self.buildings.get(current_activity):
            student.schedule.pop(0)
            if student.schedule:
                next_activity = student.schedule[0]
                student.destination = self.buildings.get(next_activity)
                student.current_path = self.find_shortest_path(student.position, student.destination)
                student.move_start_time = self.virtual_time
    ...
    def update(self):
        ...
        for student in self.students:
            ...
            self.update_student_activity(student)
